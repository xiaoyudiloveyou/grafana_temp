...
describe('InfluxQuery', function () {
    ...
    describe('render series with mesurement only', function () {
        it('should generate correct query', function () {
            var query = new InfluxQueryModel({
                refId: 'A',
                measurement: 'cpu',
            }, templateSrv, {});
    ...
    describe('field name with single quote should be escaped and', function () {
        it('should generate correct query', function () {
            var query = new InfluxQueryModel({
                refId: 'A',
                measurement: 'cpu',
            ...
            var queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "cpu" WHERE ("name" = \'Let\\\'s encrypt.\' OR "hostname" = \'server2\') AND ' +
                '$timeFilter GROUP BY time($__interval)');
        });
    });
    ...
    describe('when adding select part', function () {
        ...
        describe('when render adhoc filters', function () {
            it('should generate correct query segment', function () {
                var query = new InfluxQueryModel({ refId: 'A', measurement: 'cpu' }, templateSrv, {});
