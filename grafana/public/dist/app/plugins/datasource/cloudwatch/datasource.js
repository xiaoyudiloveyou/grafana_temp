...
var CloudWatchDatasource = /** @class */ (function (_super) {
    tslib_1.__extends(CloudWatchDatasource, _super);
    /** @ngInject */
    ...
    CloudWatchDatasource.prototype.performTimeSeriesQuery = function (request) {
        return this.awsRequest('/api/tsdb/query', request).then(function (res) {
            ...
            if (res.results) {
                try {
                    for (var _c = tslib_1.__values(request.queries), _d = _c.next(); !_d.done; _d = _c.next()) {
                        var query = _d.value;
                        var queryRes = res.results[query.refId];
                        if (queryRes) {
                            try {
                                for (var _e = (e_2 = void 0, tslib_1.__values(queryRes.series)), _f = _e.next(); !_f.done; _f = _e.next()) {
                                    var series = _f.value;
    ...
    CloudWatchDatasource.prototype.expandTemplateVariable = function (targets, scopedVars, templateSrv) {
        ...
        return _.chain(targets)
            ...
            var variableIndex = _.keyBy(templateSrv.variables, 'name');
