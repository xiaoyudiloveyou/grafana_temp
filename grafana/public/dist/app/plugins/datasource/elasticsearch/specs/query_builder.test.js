...
describe('ElasticQueryBuilder', function () {
    ...
    it('with defaults on es5.x', function () {
        var builder5x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 5,
        });
        var query = builder5x.build({
            metrics: [{ type: 'Count', id: '0' }],
            timeField: '@timestamp',
            bucketAggs: [{ type: 'date_histogram', field: '@timestamp', id: '1' }],
        });
    ...
    it('with term agg and order by term', function () {
        var query = builder.build({
    ...
    it('with term agg and order by term on es6.x', function () {
        var builder6x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 60,
        });
        var query = builder6x.build({
            metrics: [{ type: 'count', id: '1' }, { type: 'avg', field: '@value', id: '5' }],
            bucketAggs: [
                {
                    type: 'terms',
    ...
    it('getTermsQuery(default case) es<6.0 should set asc sorting on _term', function () {
        var query = builder.getTermsQuery({});
    ...
    it('getTermsQuery(order:desc) es<6.0 should set desc sorting on _term', function () {
        var query = builder.getTermsQuery({ order: 'desc' });
        expect(query.aggs['1'].terms.order._term).toBe('desc');
        expect(query.aggs['1'].terms.order._key).toBeUndefined();
        expect(query.aggs['1'].terms.order._count).toBeUndefined();
    });
    it('getTermsQuery(orderBy:doc_count) es<6.0 should set desc sorting on _count', function () {
    ...
    it('getTermsQuery(orderBy:doc_count, order:asc) es<6.0 should set asc sorting on _count', function () {
    ...
    it('getTermsQuery(default case) es6.x should set asc sorting on _key', function () {
        var builder6x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 60,
        });
        var query = builder6x.getTermsQuery({});
        expect(query.aggs['1'].terms.order._term).toBeUndefined();
        expect(query.aggs['1'].terms.order._key).toBe('asc');
        expect(query.aggs['1'].terms.order._count).toBeUndefined();
    });
    it('getTermsQuery(order:desc) es6.x should set desc sorting on _key', function () {
        var builder6x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 60,
        });
        var query = builder6x.getTermsQuery({ order: 'desc' });
        expect(query.aggs['1'].terms.order._term).toBeUndefined();
        expect(query.aggs['1'].terms.order._key).toBe('desc');
        expect(query.aggs['1'].terms.order._count).toBeUndefined();
    });
    it('getTermsQuery(orderBy:doc_count) es6.x should set desc sorting on _count', function () {
        var builder6x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 60,
        });
        var query = builder6x.getTermsQuery({ orderBy: 'doc_count' });
        expect(query.aggs['1'].terms.order._term).toBeUndefined();
        expect(query.aggs['1'].terms.order._key).toBeUndefined();
        expect(query.aggs['1'].terms.order._count).toBe('desc');
    });
    it('getTermsQuery(orderBy:doc_count, order:asc) es6.x should set asc sorting on _count', function () {
        var builder6x = new ElasticQueryBuilder({
            timeField: '@timestamp',
            esVersion: 60,
        });
        var query = builder6x.getTermsQuery({ orderBy: 'doc_count', order: 'asc' });
        expect(query.aggs['1'].terms.order._term).toBeUndefined();
        expect(query.aggs['1'].terms.order._key).toBeUndefined();
        expect(query.aggs['1'].terms.order._count).toBe('asc');
    });
    // Logs query
    it('getTermsQuery should request documents and date histogram', function () {
        var query = builder.getLogsQuery({});
