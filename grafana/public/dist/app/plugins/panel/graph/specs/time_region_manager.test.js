...
describe('TimeRegionManager', function () {
        ...
        describe(desc, function () {
            ...
            ctx.printScenario = function () {
                ...
                ctx.options.grid.markings.forEach(function (m, i) {
                    console.log("Marking (" + i + "): from=" + dateTime(m.xaxis.from).format() + ", to=" + dateTime(m.xaxis.to).format() + ", color=" + m.color);
                });
            };
            func(ctx);
        });
    ...
    describe('When colors missing in config', function () {
        plotOptionsScenario('should not throw an error when fillColor is undefined', function (ctx) {
            ...
            var from = dateTime('2018-01-01T00:00:00+01:00');
            var to = dateTime('2018-01-01T23:59:00+01:00');
    ...
    describe('When creating plot markings using local time', function () {
        ...
        plotOptionsScenario('for time from region', function (ctx) {
            ...
            var from = dateTime('2018-01-01T00:00+01:00');
            var to = dateTime('2018-01-03T23:59+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
        ...
        plotOptionsScenario('for time to region', function (ctx) {
            ...
            var from = dateTime('2018-02-01T00:00+01:00');
            var to = dateTime('2018-02-03T23:59+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
        ...
        plotOptionsScenario('for time from/to region', function (ctx) {
            ...
            var from = dateTime('2018-12-01T00:00+01:00');
            var to = dateTime('2018-12-03T23:59+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
        ...
        plotOptionsScenario('for time from/to region crossing midnight', function (ctx) {
            var regions = [{ from: '22:00', to: '00:30', fill: true, colorMode: 'red' }];
            var from = dateTime('2018-12-01T12:00+01:00');
            var to = dateTime('2018-12-04T08:00+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
            it('should add one fill between 22:00 and 00:30 each day', function () {
                var markings = ctx.options.grid.markings;
        ...
        plotOptionsScenario('for day of week from/to region', function (ctx) {
            ...
            var from = dateTime('2018-01-01T18:45:05+01:00');
            var to = dateTime('2018-01-22T08:27:00+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
        ...
        plotOptionsScenario('for day of week from/to time region', function (ctx) {
            ...
            var from = dateTime('2018-12-07T12:51:19+01:00');
            var to = dateTime('2018-12-10T13:51:29+01:00');
            ctx.setup(regions, from, to);
        ...
        plotOptionsScenario('for day of week from/to time region with daylight saving time', function (ctx) {
            ...
            var from = dateTime('2018-03-17T06:00:00+01:00');
            var to = dateTime('2018-04-03T06:00:00+02:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
        ...
        plotOptionsScenario('for each day of week with winter time', function (ctx) {
            var regions = [{ fromDayOfWeek: 7, toDayOfWeek: 7, fill: true, colorMode: 'red' }];
            var from = dateTime('2018-10-20T14:50:11+02:00');
            var to = dateTime('2018-11-07T12:56:23+01:00');
            ctx.setup(regions, from, to);
            it('should add 3 markings', function () {
                expect(ctx.options.grid.markings.length).toBe(3);
            });
